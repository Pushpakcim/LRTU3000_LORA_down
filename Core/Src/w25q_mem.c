...
struct FlashData_full
{
	char Flash_array[4096];
};
/**
...
unsigned int Flash_status,Flash_status1,Flash_status2;
struct FlashData Struct_data;
struct FlashData Struct_data1;
struct FlashData_full struct_array,struct_array1;
/// @}
...
void FLASH_start()
{
	osThreadDef(FLASHTask, StartFLASHTask, osPriorityBelowNormal, 0, 128*8); //512
	ADC_TaskHandle = osThreadCreate(osThread(FLASHTask), NULL);
}

/**************************************************************************//**
 * Function name 	: StartFLASHTask
 * arguments		: 1)
 * return 		 	:
 * Note				:
 * 					:
 * 					:
 *****************************************************************************/
unsigned char tempChar,Flash_Status=0;
unsigned int flash_count;
int j =0;

unsigned char flag_flashSaveRecipe=0,flag_flashUpdateEPROM_General=0,flag_flashUpdateEPROM_AI_Calibration=0,flag_flashUpdateEPROM_Schedule=0,flag_flashUpdateEPROM_Modbus_Quary_Detail=0,
flag_flashUpdateEPROM_Frequent=0,flag_flashUpdateEPROM_LORA=0;

unsigned int flag_flashSaveRecipe_WaitCounter=10,flag_flashUpdateEPROM_General_WaitCounter=10,flag_flashUpdateEPROM_AI_Calibration_WaitCounter=10,flag_flashUpdateEPROM_Schedule_WaitCounter=10,
		flag_flashUpdateEPROM_Modbus_Quary_Detail_WaitCounter=10,flag_flashUpdateEPROM_PCBPLC_GENERAL_REG_WaitCounter=0,flag_flashUpdateEPROM_Frequent_WaitCounter=0,flag_flashUpdateEPROM_LORA_WaitCounter=10;
...
W25Q_STATE W25Q_Init(void) {
	...
	if (state != W25Q_OK)
		return state;
	// u can check id here
	sprintf((char *)print, " W25Q Read ID : %d\r\n",id);
	WriteLog(1, print, 1);
	memset(print,0,sizeof(print));

	// read chip's state to private lib's struct
	state = W25Q_ReadStatusStruct(&status_q25W65);
	if (state != W25Q_OK)
		return state;
...
/**
 * @brief W25Q number of sector erase (*4 KB)
 * Func to erase number of sector
 *
 * @note Should be executed before writing
 * @param[in] startAddress start address
 * @param[in] total_no_sec number of sector erase after starting sector
 * @return W25Q_STATE enum
 */

W25Q_STATE Erase_RECsector(unsigned int startAddress,int total_no_sec)
{
	W25Q_STATE FLASH_ret = W25Q_OK;
	for (int no_sec = 0 ;no_sec < total_no_sec ; no_sec++)
	{
		FLASH_ret = W25Q_EraseSector(startAddress + (no_sec*4096));
...
/**************************************************************************//**
 * Function name 	: page_to_addr
 * arguments		: 1) pageNum Number of page
 * 		 			: 2) pageShift Byte to shift inside page
 * return			: byte-address
 * Note				:
 * 					:
 *****************************************************************************/
 ...
 */
/**************************************************************************//**
 * Function name 	: W25Q_Erase_Write_One_Sector
 * arguments		: 1) buf Data to be written (single)
 * 		 			: 2) pageShift Byte shift inside page (0..255)
 * 		 			: 3) pageNum Page number (0..PAGE_COUNT-1)
 * return			: W25Q_STATE enum
 * Note				:
 *****************************************************************************/
W25Q_STATE W25Q_Erase_Write_One_Sector(u8_t *buf, u16_t len, u32_t pageShift){

	uint8_t status1,status2;
	uint32_t end_addr, current_size, current_addr=0,last_addr = 0;

	//Sector_Erase = (len-1)/SECTOR_COUNT;

	current_addr = pageShift;
	last_addr = current_addr+len;

	while(current_addr < last_addr)
	{
		status1=W25Q_EraseSector(current_addr); //SECTOR_COUNT
		if(status1!= W25Q_OK){
			return W25Q_PARAM_ERR;
		}
		current_addr+=SECTOR_COUNT;
	}
//	for(int i=0;i<=Sector_Erase;i++)
//	{
//		status1=W25Q_EraseSector(current_addr); //SECTOR_COUNT
//		if(status1!= W25Q_OK){
//			return W25Q_PARAM_ERR;
...
//	}
	...
	current_size = MEM_PAGE_SIZE - (pageShift % MEM_PAGE_SIZE);
	...
	/* Initialize the adress variables */
		current_addr = pageShift;
		end_addr = pageShift + len;  //+1 for null
	do
	{
		status2 = W25Q_ProgramRaw(buf, current_size, current_addr);
		if(status2!= W25Q_OK){
			return W25Q_PARAM_ERR;
		}
		HAL_Delay(3);
		...
		buf += current_size;
		current_size = ((current_addr + MEM_PAGE_SIZE) > end_addr) ? (end_addr - current_addr) : MEM_PAGE_SIZE;
...
/**
 * @brief Erase, Write given data size in flash
 * Translate void
 *
 * @param[in] buf Data to be written (single)
 * @param[in] pageShift Byte shift inside page (0..255)
 ...
 */
/**************************************************************************//**
 * Function name 	: W25Q_Write_continous
 * arguments		: 1) buf Data to be written (single)
 * 		 			: 2) pageShift Byte shift inside page (0..255)
 * 		 			: 3) pageNum Page number (0..PAGE_COUNT-1)
 * return			: W25Q_STATE enum
 * Note				:
 *****************************************************************************/
//W25Q_STATE W25Q_Write_given_data(u8_t *buf, u16_t len, u32_t pageShift)
...
//	status1=W25Q_EraseSector(pageShift);
//	if(status1!= W25Q_OK){
//		return W25Q_PARAM_ERR;
//	}
